...
| `LAKEKEEPER__ENABLE_HDFS_WITH_SYSTEM_CREDENTIALS`           | <nobr>`true`<nobr> | Enables Lakekeeper's HDFS support including system Kerberos credentials (from the ticket cache specified by KRB5CCNAME) when accessing HDFS storage locations. This feature is disabled by default to prevent accidental access to restricted HDFS locations. To enable HDFS with system credentials, set this to `true`. Default: `false` (HDFS system credentials disabled) |
