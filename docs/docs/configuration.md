...
| Variable                                                    | Example            | Description |
|-------------------------------------------------------------|--------------------|-----|
| `LAKEKEEPER__S3_DISABLE_SYSTEM_CREDENTIALS`                 | <nobr>`true`<nobr> | By default users can create Warehouses using storage credentials configured on the system / instance itself (i.e. `AWS_*` environment variables or EC2 instance profiles). This might be unwanted in some cases if these credentials have access to locations that users should not be able to read or store tables in. When setting `LAKEKEEPER__S3_DISABLE_SYSTEM_CREDENTIALS` to `true`, Lakekeeper does not allow the creation of Warehouses that use system credentials. Default: `false` (System credentials enabled) |
| `LAKEKEEPER__S3_DISABLE_DIRECT_SYSTEM_CREDENTIALS`          | <nobr>`true`<nobr> | If a Warehouse is created using system credentials, by default, users are not required to specify a `assume-role-arn`. Lakekeeper then uses the configured serviceaccount to access files directly. In managed settings, the configured system account should often not have any permissions itself, other than to assume roles. If `LAKEKEEPER__S3_DISABLE_SYSTEM_CREDENTIALS` is set to `true`, Lakekeeper requires users to set the `assume-role-arn` when configuring Warehouses with system credentials. Default: `false` (Credentials can be used without assuming a role) |
| `LAKEKEEPER__S3_REQUIRE_EXTERNAL_ID_FOR_SYSTEM_CREDENTIALS` | <nobr>`true`<nobr> | If true, an `external-id` is required when assuming a role with system credentials. Default: `false` |
