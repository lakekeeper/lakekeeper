...
| `LAKEKEEPER__CEDAR__EXTERNALLY_MANAGED_USER_AND_ROLES`   | `false`                                               | If `true`, Lakekeeper does not provide `Lakekeeper::Role` and `Lakekeeper::User` entities from the user's token to Cedar, and expects that all roles and users are managed externally via entities.json. If `false` (default), Lakekeeper provides `Lakekeeper::Role` and `Lakekeeper::User` entities to Cedar based on information from the user's token. |
| `LAKEKEEPER__CEDAR__SCHEMA_FILE`                         | `/path/to/custom/schema.cedarschema`                  | Optional path to a custom Cedar schema file. If provided, this schema will be used instead of the embedded default schema. Useful for extending or customizing the Cedar schema. Compatability with the Lakekeeper schema must be ensured for all entities provided by Lakekeeper (Server, Project, Namespace, Table, View. User & Role if externally managed roles is `false`). |
