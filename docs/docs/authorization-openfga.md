...
- **User ids are taken as given.** OpenFGA does not check that a user exists, so granting to a mistyped user id succeeds silently. Role ids are checked: a `/grants` write below the server rejects a role outside the project with `GrantRoleNotInProject`. Server grants have no project to check against and skip that lookup.
