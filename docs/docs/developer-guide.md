...
**Which bump.** Wire enums reach the log as string *values*, so renaming a variant changes the payload even though no key moves.

| Change                                                                                          | Bump  |
|-------------------------------------------------------------------------------------------------|-------|
| A field added                                                                                   | minor |
| A new action, entity type, entity field or action context key                                   | minor |
| A field removed, renamed or retyped                                                             | major |
| A wire value renamed — `entity_type`, `decision`, `actor_type`, `action_name`, a `Valuable` tag | major |
| A fixture edited, renamed, added or dropped, with no change to what the server emits            | none  |

A major bump resets the minor, so `1.4` becomes `2.0` and not `2.1`. Skipping numbers, going backwards, bumping both halves at once and removing `AUDIT_FORMAT` are all rejected.

#### Steps

**1. Make the change.** Adding a key, an action or a variant is a compile error by design: the build stops where the wire name and its documentation have to be chosen.

- `EntityField::as_str` (`events/context.rs`) — an entity field key.
- `EntityType::as_str` (`events/context.rs`) — an `entity_type` value.
- `ActionContextKey::as_str` (`events/context.rs`) — an action context key.
...
The first four fail `cargo build`. The last fails `cargo test` and `just check` but not a default `cargo build`, because `contract` is gated `#[cfg(any(test, feature = "test-utils"))]`. CI runs both.

**Never add a `_ =>` arm to those matches.** The missing wildcard is the mechanism: with one, a new entity key silently takes another key's wire name and a new action silently emits no context, and no fixture notices, because a fixture can only exercise a variant that already exists. They carry `#[deny(clippy::wildcard_enum_match_arm)]`, so a wildcard fails `just check` and CI, though not a bare `cargo build`. A wildcard *alongside* the full list is caught by rustc's `unreachable_patterns`; the dangerous edit is replacing arms with one. The exhaustive lists make some of these functions long — one carries `#[allow(clippy::too_many_lines)]` — so do not shorten them by collapsing the list. Not every `action_descriptor` impl carries the deny yet, and `CatalogAction` is public, so authorizer crates have impls this repository cannot see; add the deny when you next touch an unprotected one.

**2. Regenerate: `just update-audit-fixtures`.** Read the diff. It is exactly what a consumer's pipeline will see, so anything in it you did not intend is the bug.

**3. Set `AUDIT_FORMAT`.** A major bump also renames the fixture directory: `git mv fixtures/v1 fixtures/v2`, then regenerate again. No code change is needed — the tests derive the directory from `AUDIT_FORMAT` and fail until it exists. Keep exactly one directory. A fixture records what the current code emits, so a retired format can neither be reproduced nor kept passing; it lives in the release notes and in git history instead.
...
**5. Check the bump: `just check-audit-format-bump`.** CI runs it on every pull request.

**6. Say so in the pull request** — an `audit` mention in the title, or a `## Release notes` section in the body describing the change in consumer terms. Major bumps must appear in the release notes.

#### What the checks cover

`AUDIT_FORMAT` is a claim and the fixtures are the evidence. The fixture tests compare emitted records against committed JSON and classify any difference, but only until you regenerate: afterwards they pass whether you bumped correctly, wrongly, or not at all. `check-audit-format-bump` closes that gap by comparing the committed fixtures and the action-name manifest either side of the merge base and requiring the bump to match. Its decision table is self-tested with `--self-test`.

Comparison is on key paths and JSON types rather than values, and containers record their own type, so `{}`, `[]` and an absent field stay distinguishable. Two consequences. A changed fixture *value* is reported but not classified — nothing can tell a renamed wire value from a more realistic test input, so the checker says so and leaves the decision to you. And fixtures present in only one revision are not compared, so renaming or removing one costs the checker its verdict, which it reports rather than reading as "nothing changed".

Checked exhaustively:

- **Entity field keys, `entity_type` values, action context keys** — from the type system. The enums are closed, so a key cannot exist without a `match` arm and a documented row.
- **Variants of the derived audit enums** — via the tag functions above.
- **`action_name` removals, per enum.** Each action enum derives its names with `strum`'s `VariantNames`, committed as `audit/action_names.json`: keyed by enum, plus a `literals` list for hand-written descriptors. Per enum matters because most verbs are shared, so a flattened comparison would miss one enum renaming a verb the others still use. Additions are not a format change. Three things sit outside the manifest: an authorizer crate's own actions, since `CatalogAction` is public with a blanket `APIEventActions` impl; a brand-new action *enum*, until it is added to `derived_action_name_enums` in the audit test module; and a hand-written `ActionDescriptor`, covered only if its name is in `LITERAL_ACTION_NAMES`, which `grep -rn 'action_name("' crates/lakekeeper/src` will tell you.
