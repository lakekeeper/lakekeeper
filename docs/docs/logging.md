...
| `privilege_source`     | String          | How the decision was reached: `"authorizer"` (configured Authorizer backend), `"instance_admin"` (caller listed in `LAKEKEEPER__INSTANCE_ADMINS` — control-plane bypass), or `"internal"` (in-process call — full bypass). See [Instance Admins](./authorization.md#instance-admins). |
