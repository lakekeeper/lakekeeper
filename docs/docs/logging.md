...
| `break_glass`          | String          | Optional; present only when the caller sent the `x-break-glass` request header, which nearly no request does. The reason the caller stated for marking the request an emergency override, recorded verbatim and truncated to 256 bytes. **Client-supplied and unverified** — see below. |
