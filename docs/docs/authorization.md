...
- **Data-plane operations** — `CatalogTableAction::ReadData` and
  `CatalogTableAction::WriteData` still route through the configured
  Authorizer. If the instance admin does not hold the relevant grants,
  reads and writes of table row data are denied.
