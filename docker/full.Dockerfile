...
RUN apt-get update -qq && \
  DEBIAN_FRONTEND=noninteractive apt-get install -yqq cmake curl build-essential libpq-dev pkg-config make perl wget zip unzip clang-19 --no-install-recommends && \
