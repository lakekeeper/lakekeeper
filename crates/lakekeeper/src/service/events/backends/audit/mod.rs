...
impl Valuable for Authorization {
    ...
    ///
    /// Unifying the three is an open issue for the next major version bump — see the audit-log
...
/// open issue for the next major version bump — see the audit-log section of
...
pub mod contract {
    ...
    use crate::service::events::context::{ActionContextKey, EntityField, EntityType};
    ...
    /// Failure reasons that mean the request was evaluated and refused, as opposed to never
    /// having reached a verdict.
    const DEFINITIVE_DENIALS: &[&str] =
        &["ActionForbidden", "ResourceNotFound", "CannotSeeResource"];

    /// Strip the subscriber-owned envelope, leaving the fields the format promises.
    #[must_use]
        ...
        for field in [singular, plural] {
            match record.get(field) {
                Some(serde_json::Value::Array(items)) => {
                    for item in items {
                        out.extend(object_keys(item));
                    }
                }
                Some(value) => out.extend(object_keys(value)),
                None => {}
            }
        }
        out
    ...
    pub fn violations(record: &serde_json::Value) -> Vec<String> {
        ...
        // `entity_type` can appear at any depth: on the record's own entity, and again on
        // every per-decision entry.
        let known_type: BTreeSet<&str> = EntityType::VARIANTS.iter().map(|t| t.as_str()).collect();
        let mut stack = vec![record];
        while let Some(value) = stack.pop() {
            match value {
                serde_json::Value::Object(object) => {
                    if let Some(serde_json::Value::String(kind)) = object.get("entity_type")
        ...
        if record.get("failure_reason").is_some()
            && record.get("decision").and_then(serde_json::Value::as_str) != Some("denied")
        {
