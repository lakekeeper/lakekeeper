...
use crate::{
    ...
    api::RequestMetadata,
    service::{
        AuthZTableInfo, AuthZTabularInfo, AuthZViewInfo, CatalogBackendError,
        ...
        TableInfo, TabularIdentOrId, TabularNotFound, TaskNotFoundError,
        UnexpectedTabularInResponse, WarehouseStatus,
        authz::{
            AuthZCannotSeeView, AuthZError, AuthZViewActionForbidden, AuthZViewOps,
...
#[derive(Debug, derive_more::From)]
pub enum RequireTabularPresenceError {
    AuthZCannotSeeTable(AuthZCannotSeeTable),
...
impl From<GetTabularInfoError> for RequireTabularPresenceError {
    fn from(err: GetTabularInfoError) -> Self {
        match err {
            GetTabularInfoError::CatalogBackendError(e) => e.into(),
            GetTabularInfoError::InvalidNamespaceIdentifier(e) => e.into(),
...
impl From<GetTabularInfoByLocationError> for RequireTabularPresenceError {
    fn from(err: GetTabularInfoByLocationError) -> Self {
        match err {
            GetTabularInfoByLocationError::CatalogBackendError(e) => e.into(),
            GetTabularInfoByLocationError::InvalidNamespaceIdentifier(e) => e.into(),
...
delegate_authorization_failure_source!(RequireTabularPresenceError => {
    AuthZCannotSeeTable,
    AuthZCannotSeeView,
    CatalogBackendError,
    InvalidNamespaceIdentifier,
...
pub trait AuthZTableOps: Authorizer {
    ...
    fn require_tabulars_presence<T: AuthZTabularInfo>(
        &self,
        warehouse_id: WarehouseId,
        user_provided_tabulars: Vec<impl Into<TabularIdentOrId> + Send>,
        tabulars: Result<Vec<T>, impl Into<RequireTabularPresenceError> + Send>,
    ) -> Result<Vec<T>, RequireTabularPresenceError> {
        let tabulars = tabulars.map_err(Into::into)?;

        // Convert user-provided tabulars to TabularIdentOrId for comparison
        let user_provided: Vec<TabularIdentOrId> =
            user_provided_tabulars.into_iter().map(Into::into).collect();

        // If counts don't match, we know something is missing
        if user_provided.len() != tabulars.len() {
            // Build a set of returned tabulars by their identifiers for quick lookup
            let returned_set: HashSet<TabularIdentOrId> = tabulars
                .iter()
                .flat_map(|t| {
                    let tabular_id = BasicTabularInfo::tabular_id(t);
                    let tabular_ident = t.tabular_ident().clone();
                    // Store both ID and Ident representations
                    // Wrap in appropriate TabularIdentOwned based on tabular type
                    let wrapped_ident = match tabular_id {
                        crate::service::TabularId::Table(_) => TabularIdentOrId::Ident(
                            crate::service::TabularIdentOwned::Table(tabular_ident),
                        ),
                        crate::service::TabularId::View(_) => TabularIdentOrId::Ident(
                            crate::service::TabularIdentOwned::View(tabular_ident),
                        ),
                    };
                    vec![TabularIdentOrId::Id(tabular_id), wrapped_ident]
                })
                .collect();

            // Find the first missing tabular
            for user_tabular in &user_provided {
                if !returned_set.contains(user_tabular) {
                    // Determine if this is a table or view based on the identifier
                    return Err(match user_tabular {
                        TabularIdentOrId::Ident(crate::service::TabularIdentOwned::Table(
                            table_ident,
                        )) => AuthZCannotSeeTable::new_not_found(
                            warehouse_id,
                            crate::service::TableIdentOrId::Ident(table_ident.clone()),
                        )
                        .into(),
                        TabularIdentOrId::Id(crate::service::TabularId::Table(table_id)) => {
                            AuthZCannotSeeTable::new_not_found(
                                warehouse_id,
                                crate::service::TableIdentOrId::Id(*table_id),
                            )
                            .into()
                        }
                        TabularIdentOrId::Ident(crate::service::TabularIdentOwned::View(
                            view_ident,
                        )) => AuthZCannotSeeView::new_not_found(
                            warehouse_id,
                            crate::service::ViewIdentOrId::Ident(view_ident.clone()),
                        )
                        .into(),
                        TabularIdentOrId::Id(crate::service::TabularId::View(view_id)) => {
                            AuthZCannotSeeView::new_not_found(
                                warehouse_id,
                                crate::service::ViewIdentOrId::Id(*view_id),
                            )
                            .into()
                        }
                    });
...
mod tests {
    ...
    #[sqlx::test]
    async fn test_require_tabulars_presence_all_present(pool: PgPool) {
        let authz = HidingAuthorizer::new();
        let (ctx, warehouse_resp) = SetupTestCatalog::builder()
            .pool(pool)
            .authorizer(authz.clone())
        ...
        let prefix = warehouse_resp.warehouse_id.to_string();
        let _ns = create_ns(ctx.clone(), prefix.clone(), "test_ns".to_string()).await;
        let _table1 = create_table(ctx.clone(), &prefix, "test_ns", "t1", false)
            .await
            .unwrap();
        let _table2 = create_table(ctx.clone(), &prefix, "test_ns", "t2", false)
        ...
        // All tabulars are present, should succeed
        let user_provided = vec![
            TabularIdentOrId::Ident(TabularIdentOwned::Table(table1_ident)),
            TabularIdentOrId::Ident(TabularIdentOwned::Table(table2_ident)),
            TabularIdentOrId::Ident(TabularIdentOwned::View(view1_ident)),
        ];

        let result = authz.require_tabulars_presence(
            warehouse_resp.warehouse_id,
            user_provided,
            Ok::<_, RequireTabularPresenceError>(infos),
        );

        assert!(result.is_ok());
        let tabulars = result.unwrap();
        assert_eq!(tabulars.len(), 3);
    }

    #[sqlx::test]
    async fn test_require_tabulars_presence_some_missing(pool: PgPool) {
        let authz = HidingAuthorizer::new();
        let (ctx, warehouse_resp) = SetupTestCatalog::builder()
            .pool(pool)
            .authorizer(authz.clone())
        ...
        // User requested 3 tabulars but only 2 exist
        let user_provided = vec![
            TabularIdentOrId::Ident(TabularIdentOwned::Table(table1_ident)),
            TabularIdentOrId::Ident(TabularIdentOwned::Table(table2_ident.clone())),
            TabularIdentOrId::Ident(TabularIdentOwned::View(view1_ident)),
        ];
        ...
        // The error should be AuthZCannotSeeTable for the missing table
        match err {
            RequireTabularPresenceError::AuthZCannotSeeTable(e) => {
                assert_eq!(e.warehouse_id, warehouse_resp.warehouse_id);
                assert_eq!(e.table, TableIdentOrId::Ident(table2_ident));
                assert!(e.internal_resource_not_found);
            }
            _ => panic!("Expected AuthZCannotSeeTable error, got: {err:?}"),
        }
    }

    #[sqlx::test]
    async fn test_require_tabulars_presence_empty_input(pool: PgPool) {
        let authz = HidingAuthorizer::new();
        let (ctx, warehouse_resp) = SetupTestCatalog::builder()
            .pool(pool)
            .authorizer(authz.clone())
        ...
        let user_provided: Vec<TabularIdentOrId> = vec![];
        let infos: Vec<crate::service::ViewOrTableInfo> = vec![];
    ...
    #[sqlx::test]
    async fn test_require_tabulars_presence_backend_error(pool: PgPool) {
        let authz = HidingAuthorizer::new();
        let (_ctx, warehouse_resp) = SetupTestCatalog::builder()
            .pool(pool)
            .authorizer(authz.clone())
            .storage_profile(memory_io_profile())
            .build()
        ...
        let user_provided = vec![TabularIdentOrId::Ident(TabularIdentOwned::Table(
            table1_ident,
        ))];

        // Simulate a backend error
        let backend_error =
            CatalogBackendError::new_unexpected(std::io::Error::other("Database connection lost"));

        let result: Result<Vec<crate::service::ViewOrTableInfo>, RequireTabularPresenceError> =
            authz.require_tabulars_presence(
                warehouse_resp.warehouse_id,
                user_provided,
                Err(backend_error),
            );
        ...
        match result.unwrap_err() {
            RequireTabularPresenceError::CatalogBackendError(_) => {}
            other => panic!("Expected CatalogBackendError, got: {other:?}"),
        }
    }
}
